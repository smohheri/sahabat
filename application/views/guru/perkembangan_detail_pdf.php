...
<table style="width:100%; border-collapse: collapse; margin-bottom: 14px; font-size: 11px;">
    ...
    <tr>
        ...
        <td style="padding: 4px 6px;">:
            <?php echo !empty($assessor_name) ? htmlspecialchars($assessor_name, ENT_QUOTES, 'UTF-8') : '-'; ?>
        </td>
    </tr>
</table>
...
<h3 style="margin: 10px 0 6px; font-size: 13px; border-bottom: 1px solid #ddd; padding-bottom: 4px;">Ringkasan Riwayat
    Penilaian</h3>
<table border="1" cellspacing="0" cellpadding="4" style="width:100%; border-collapse: collapse; font-size: 10px;">
    <thead style="background: #f1f1f1;">
        <tr>
            ...
            <th style="width: 14%;">Minggu/Bulan/Tahun</th>
            <th style="width: 10%;">Rata-rata</th>
            <th style="width: 10%;">Total Indikator</th>
    ...
    <tbody>
        <?php if (!empty($history_rows)): ?>
            <?php foreach ($history_rows as $item): ?>
                <tr>
                    <td style="text-align:center;">#<?php echo (int) $item->id_assessment; ?></td>
                    <td style="text-align:center;">
                        <?php echo !empty($item->assessment_date) ? date('d-m-Y', strtotime($item->assessment_date)) : '-'; ?>
                    </td>
                    <td style="text-align:center;">
                        <?php echo (int) $item->week_number; ?>/<?php echo (int) $item->month; ?>/<?php echo (int) $item->year; ?>
                    </td>
                    <td style="text-align:center;">
                        <?php echo $item->avg_score !== null ? number_format((float) $item->avg_score, 2) : '-'; ?>
                    </td>
                    <td style="text-align:center;"><?php echo (int) $item->total_indicator; ?></td>
                    <td style="text-align:center;"><?php echo ucfirst((string) $item->status); ?></td>
                    <td><?php echo !empty($item->notes) ? htmlspecialchars(strip_tags($item->notes), ENT_QUOTES, 'UTF-8') : '-'; ?>
                    </td>
                </tr>
            <?php endforeach; ?>
        <?php else: ?>
            <tr>
                <td colspan="7" style="text-align:center; color:#777;">Belum ada riwayat penilaian pada periode ini.</td>
            </tr>
        <?php endif; ?>
    </tbody>
</table>
...
<div style="margin-top: 24px; text-align: right; font-size: 11px;">
    <p style="margin: 0;">Dicetak pada: <?php echo tanggal_indo($printed_at); ?></p>
</div>
